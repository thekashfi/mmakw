...
<head>
    ...
    <title>@if(app()->getLocale()=="en") {{$settingInfo->name_en}} @else {{$settingInfo->name_ar}} @endif | @if(app()->getLocale()=="en") {{$practicedetails->title_en}} @else  {{$practicedetails->title_ar}} @endif</title>
    
    <meta name="description" content="@if(app()->getLocale()=="en")@if($practicedetails->seo_description_en){{$practicedetails->seo_description_en}}@else    {{$settingInfo->seo_description_en}}@endif @else @if($practicedetails->seo_description_ar){{$settingInfo->seo_description_ar}}@else{{$settingInfo->seo_description_ar}} 
@endif @endif" />
    <meta name="abstract" content="@if(app()->getLocale()=="en") {{$settingInfo->seo_description_en}} @else {{$settingInfo->seo_description_ar}} @endif">
    <meta name="keywords" content="@if(app()->getLocale()=="en")@if($practicedetails->seo_keywords_en){{$settingInfo->seo_keywords_en}}@else {{$settingInfo->seo_keywords_en}} @endif @else @if($practicedetails->seo_keywords_en) {{$practicedetails->seo_keywords_en}} @else {{$settingInfo->seo_keywords_ar}} @endif @endif" />
...
<body id="home" data-appear-top-offset='-300'>
    ...
    <!-- end preloader -->
        ...
        <section class="inner_page" style="background:url({{url('assets/images/slider/slide-3.jpg')}}) no-repeat;">
            <h2 class="text-center">@if(app()->getLocale()=="en") {{$practicedetails->title_en}} @else  {{$practicedetails->title_ar}} @endif</h2>
        </section>
        <!-- end of hero slider -->
            ...
            <div class="container" >
                <div class="row animatedParent">
                    ...
                    <div class="col col-md-3 col-xs-12 col-sm-12">
                        <h4>{{__('webMessage.practiceareas')}}</h4>
                        <ul class="myaccount">	
                         @if(count($practiceareaMenus))	
                            @foreach($practiceareaMenus as $practiceareaMenu)	
                            @if(app()->getLocale()=="en")			
                            <li class="my_active"><a  href="{{url('/practice/'.$practiceareaMenu->slug)}}"><i class="ti-angle-double-right myarrow"></i> @if(app()->getLocale()=="en") {{$practiceareaMenu->menu_name_en}} @else  {{$practiceareaMenu->menu_name_ar}} @endif </a></li>
                            @else
                            <li><a href="{{url('/practice/'.$practiceareaMenu->slug)}}">@if(app()->getLocale()=="en") {{$practiceareaMenu->menu_name_en}} @else  {{$practiceareaMenu->menu_name_ar}} @endif <i class="ti-angle-double-left myarrow"></i></a></li>
                            @endif
                            @endforeach
                         @endif   
                        </ul>
