...
<body id="home" data-appear-top-offset='-300'>
    ...
    <!-- end preloader -->
        ...
        <section class="inner_page" style="background:url({{url('assets/images/slider/slide-3.jpg')}}) no-repeat;">
            <h2 class="text-center">{{__('webMessage.dashboard')}}</h2>
        </section>
        <!-- end of hero slider -->
            ...
            <div class="container">
                <div class="row animatedParent">
                    ...
                    <div class="col col-md-9 col-xs-12 col-sm-12 mobi-t-20">
                     <p><b>{{__('webMessage.name')}} : </b>{{Auth::guard('webs')->user()->name}}</p>
					@if(Auth::guard('webs')->user()->mobile1)
                    <p><b>{{__('webMessage.mobile')}} : </b> {{Auth::guard('webs')->user()->mobile1}}
                    @if(Auth::guard('webs')->user()->mobile2) , {{Auth::guard('webs')->user()->mobile2}} @endif
                    @if(Auth::guard('webs')->user()->mobile3) , {{Auth::guard('webs')->user()->mobile3}} @endif
                    </p>
                    @endif	
                    @if(Auth::guard('webs')->user()->email)
                    <p><b>{{__('webMessage.email')}} : </b> {{Auth::guard('webs')->user()->email}}</p>
                    @endif	
                    @if(Auth::guard('webs')->user()->username)
                    <p><b>{{__('webMessage.username')}} : </b> {{Auth::guard('webs')->user()->username}}</p>
                    @endif	
                    
                    @if(Auth::guard('webs')->user()->created_at)
                    <p><b>{{__('webMessage.created')}} : </b> {{ \Carbon\Carbon::parse(Auth::guard('webs')->user()->created_at)->diffForHumans() }}</p>
                    @endif	
                    @php
                    $countUnSeeen = App\Http\Controllers\accountController::isReadUpdatesCount(Auth::guard('webs')->user()->id);
                    @endphp
                    <p><b>{{__('webMessage.unseen')}} : </b> {{$countUnSeeen}}</p>
					</div>
                </div>
            </div> <!-- end container -->
        </section>
