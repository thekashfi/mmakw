...
<head>
    ...
    <title>@if(app()->getLocale()=="en") {{$settingInfo->name_en}} @else {{$settingInfo->name_ar}} @endif | {{__('webMessage.casesupdates')}}</title>
            ...
            <div class="container">
                <div class="row animatedParent">
                    ...
                    <div class="col col-md-9 col-xs-12 col-sm-12 mobi-t-20">
                        ...
                        <div class="col col-md-6 font-weight-bold"><span class="my_title">{{__('webMessage.casesupdates')}}</span></div>
                        <div class="col col-md-6">
                         
                        <form action="{{route('search')}}" method="POST" role="search">
                        <div class="row">
                        {{ csrf_field() }}
                        <div class="col col-md-8 col-sm-8 col-xs-7">
                        <input placeholder="{{__('webMessage.entersearchkeywords')}}" name="q" id="search_key" type="text" class="my_input" value="@if(request()->q) {{request()->q}} @endif">
                        </div>
                        <div class="col col-md-4 col-sm-4 col-xs-3">
                        <button class="mybutton">{{__('webMessage.search_btn')}}</button>
                        </div>
                        </div>
                        </form>
				  ...
				  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="table table-bordered table-hover table-center">
				  <tr>
					<th width="10">#</th>
					<th >{{__('webMessage.title')}}</th>
                    <th width="100">{{__('webMessage.unseen')}}</th>
                    <th width="120">{{__('webMessage.date')}}</th>
					<th width="110">{{__('webMessage.action')}}</th>
				  </tr>
                @if(!empty($caselists))
                @php $i=1; @endphp
                @foreach($caselists as $caselist) 
                @php 
                $isread = App\Http\Controllers\accountController::isReadUpdates($caselist->id);
                @endphp 
                
				  <tr @if($isread>0) class="bg-info" @endif >
					<td >{{$i}}</td>
					<td >@if(app()->getLocale()=="en") {{$caselist->title_en}} @else {{$caselist->title_ar}} @endif</td>
                    <td>{{$isread}}</td>
                    <td >{{$caselist->case_date}}</td>
					<td ><A href="{{url('casesupdates-details/'.$caselist->id)}}" class="btn btn-info btn-sm">{{__('webMessage.viewdetails')}}</a></td>					
				  </tr>
                    @php $i++; @endphp
                  @endforeach
                  <tr><td colspan="5" align="center">{{$caselists->links()}}</td></tr>
				@endif   
				</table>
					</div>
                </div>
