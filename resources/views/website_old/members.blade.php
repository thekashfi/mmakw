...
<head>
    ...
    <title>@if(app()->getLocale()=="en") {{$settingInfo->name_en}} @else {{$settingInfo->name_ar}} @endif | {{__('webMessage.membershiplistings')}}</title>
...
<body id="home" data-appear-top-offset='-300'>
    ...
    <!-- end preloader -->
        ...
        <section class="inner_page" style="background:url({{url('assets/images/slider/slide-3.jpg')}}) no-repeat;">
            <h2 class="text-center">{{__('webMessage.membershiplistings')}}</h2>
        </section>
        <!-- end of hero slider -->
                ...
                <div class="row animatedParent form-group">   
                    <div class="col col-md-3">
                                <div class="img-holder">
                                    ...
                                    <img src="{{url('uploads/memberships/'.$memberslist->image)}}" alt="" title=""/>
                                    @endif
                                </div>
                    </div>
                    <div class="col col-md-9">
                    <p class="text-justify slower animated bounceInRight">
                    @if(app()->getLocale()=="en") {!!$memberslist->details_en!!} @else  {!!$memberslist->details_ar!!} @endif
                    </p>
                    </div>
                    </div>
                   @endforeach
                <div class="text-center">{{$memberslistsds->links()}}</div>
                @endif
            </div> 
