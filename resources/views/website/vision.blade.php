...
<head>
    ...
    <title>@if(app()->getLocale()=="en") {{$settingInfo->name_en}} @else {{$settingInfo->name_ar}} @endif | @if(app()->getLocale()=="en") {{$settingInfo->vision_title_en}} @else  {{$settingInfo->vision_title_ar}} @endif</title>
...
<body id="home" data-appear-top-offset='-300'>
    ...
    <!-- end preloader -->
        ...
        <section class="inner_page" style="background:url({{url('assets/images/slider/slide-3.jpg')}}) no-repeat;">
            <h2 class="text-center">@if(app()->getLocale()=="en") {{$settingInfo->vision_title_en}} @else  {{$settingInfo->vision_title_ar}} @endif</h2>
        </section>
        <!-- end of hero slider -->
