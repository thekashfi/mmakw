...
<head>
    ...
    <title>@if(app()->getLocale()=="en") {{$settingInfo->name_en}} @else {{$settingInfo->name_ar}} @endif | {{__('webMessage.teams')}}</title>
...
<body id="home" data-appear-top-offset='-300'>
    ...
    <!-- end preloader -->
        ...
        <section class="inner_page" style="background:url({{url('assets/images/slider/slide-3.jpg')}}) no-repeat;">
            <h2 class="text-center">{{__('webMessage.teams')}}</h2>
        </section>
        <!-- end of hero slider -->
        ...
        <section class="about-section" id="about">
            <div class="container">
            @if(app()->getLocale()=="en") 
            {!!$settingInfo->team_content_en!!}
            @else 
            {!!$settingInfo->team_content_ar!!} 
            @endif
            
             @if($teamlists)
                
                @foreach($teamlists as $teamlist)
                <div class="row animatedParent form-group">   
                    <div class="col col-md-3">
                                <div class="img-holder">
                                    @if($teamlist->image)
                                    <img src="{{url('uploads/teams/'.$teamlist->image)}}" alt="" title=""/>
                                    @endif
                                </div>
                    </div>
                    <div class="col col-md-9">
                    <p class="my_title">@if(app()->getLocale()=="en") {{$teamlist->name_en}} @else  {{$teamlist->name_ar}} @endif</p>
                    <p><b>{{__('webMessage.position')}} :</b>&nbsp;@if(app()->getLocale()=="en") {{$teamlist->position_en}} @else  {{$teamlist->position_ar}} @endif</p>
                    @if($teamlist->mobile)
                    <p><b>{{__('webMessage.mobile')}} :</b>&nbsp;{{$teamlist->mobile}}</p>
                    @endif
                    @if($teamlist->email)
                    <p><b>{{__('webMessage.email')}} :</b>&nbsp;{{$teamlist->email}}</p>
                    @endif
                    </div>
                    </div>
                   @endforeach
