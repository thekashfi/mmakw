...
<html lang="en">
	...
	<head>
		<base href="../../">
		<meta charset="utf-8" />
		<title>{{__('adminMessage.websiteName')}}|{{__('adminMessage.services')}}</title>
							...
							<div class="kt-container  kt-container--fluid ">
								...
								<div class="kt-subheader__toolbar">
									...
									<div class="btn-group">
										...
										<a href="{{url('gwc/services/create')}}" class="btn btn-brand btn-bold"><i class="la la-plus"></i>&nbsp;{{__('adminMessage.createnew')}}</a>
                                        @endif
							...
							<div class="kt-portlet kt-portlet--mobile">
                                ...
                                @if(auth()->guard('admin')->user()->can('services-list'))
									...
									<table class="table table-striped- table-bordered table-hover table-checkable " id="kt_table_1">
										...
										<tbody>
                                        @if(count($ServicesLists))
                                        @php $p=1; @endphp
                                        @foreach($ServicesLists as $ServicesList)
											<tr class="search-body">
												<td>{{$p}}</td>
												<td>{!! $ServicesList->title_en !!}</td>
												<td>{!! $ServicesList->title_ar !!}</td>
												<td>
                                                @if($ServicesList->image)
                                                <img src="{!! url('uploads/services/thumb/'.$ServicesList->image) !!}" width="40">
                                                @endif</td>
												<td>
                                                <span class="kt-switch"><label><input value="{{$ServicesList->id}}" {{!empty($ServicesList->is_active)?'checked':''}} type="checkbox"  id="services" class="change_status"><span></span></label></span>
                                                </td>
												<td>{!! $ServicesList->created_at !!}</td>
												<td>{!! $ServicesList->updated_at !!}</td>
                                                <td class="kt-datatable__cell">
                                                 ...
                                                 <li class="kt-nav__item"><a href="{{url('gwc/services/'.$ServicesList->id.'/edit')}}" class="kt-nav__link"><i class="kt-nav__link-icon flaticon2-contract"></i><span class="kt-nav__link-text">{{__('adminMessage.edit')}}</span></a></li>
                                                 @endif
                                                 @if(auth()->guard('admin')->user()->can('services-delete'))
                                                 <li class="kt-nav__item"><a href="javascript:;" data-toggle="modal" data-target="#kt_modal_{{$ServicesList->id}}" class="kt-nav__link"><i class="kt-nav__link-icon flaticon2-trash"></i><span class="kt-nav__link-text">{{__('adminMessage.delete')}}</span></a></li>
                                                 @endif
                                                 </ul>
                                                 </div>
                                                 </div>
                                                 ...
                                                 <!--Delete modal -->
 <div class="modal fade" id="kt_modal_{{$ServicesList->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
								<div class="modal-dialog" role="document">
									<div class="modal-content">
										...
										<div class="modal-footer">
											...
											<button type="button" class="btn btn-danger"  onClick="Javascript:window.location.href='{{url('gwc/services/delete/'.$ServicesList->id)}}'">{{__('adminMessage.yes')}}</button>
										</div>
									</div>
								</div>
							</div>
                                        ...
                                        @php $p++; @endphp
                                        @endforeach   
                                        <tr><td colspan="8" class="text-center">{{ $ServicesLists->links() }}</td></tr> 
                                        @else
