...
<html lang="en">
	...
	<head>
		<base href="../../">
		<meta charset="utf-8" />
		<title>{{__('adminMessage.websiteName')}}|{{__('adminMessage.survey')}}</title>
						...
						<div class="kt-subheader   kt-grid__item" id="kt_subheader">
							<div class="kt-container  kt-container--fluid ">
								<div class="kt-subheader__main">
									<h3 class="kt-subheader__title">{{__('adminMessage.survey')}}</h3>
									<span class="kt-subheader__separator kt-subheader__separator--v"></span>
									<div class="kt-subheader__breadcrumbs">
										<a href="{{url('home')}}" class="kt-subheader__breadcrumbs-home"><i class="flaticon2-shelter"></i></a>
										<span class="kt-subheader__breadcrumbs-separator"></span>
										<a href="javascript:;" class="kt-subheader__breadcrumbs-link">{{__('adminMessage.surveyListings')}}</a>
                                        
									</div>
								</div>
								<div class="kt-subheader__toolbar">
									<form class="kt-margin-l-20" id="kt_subheader_search_form" method="get" action="{{url('gwc/survey')}}">
								...
								<div class="kt-portlet__head kt-portlet__head--lg">
									<div class="kt-portlet__head-label">
										...
										<h3 class="kt-portlet__head-title">
											{{__('adminMessage.surveyListings')}}
										</h3>
									</div>
								</div>
                      
								<div class="kt-portlet__body">
                                @if(auth()->guard('admin')->user()->can('survey-list'))
									<!--begin: Datatable -->
									<table class="table table-striped- table-bordered table-hover table-checkable " id="kt_table_1">
										<thead>
											<tr>
                                                ...
                                                <th>{{__('adminMessage.job_title')}}</th>
												<th>{{__('adminMessage.createdat')}}</th>
												<th width="10">{{__('adminMessage.actions')}}</th>
											</tr>
										</thead>
										<tbody>
                                        @if(count($surveyLists))
                                        @php $p=1; @endphp
                                        @foreach($surveyLists as $surveyList)
											<tr class="search-body">
												<td>{{$p}}</td>
												<td>{{ $surveyList->name }}</td>
                                                <td>{{ $surveyList->email }}</td>
                                                <td>{{ $surveyList->phone }}</td>
												<td>{{ $surveyList->job_title }}</td>
                                                <td>{!! $surveyList->created_at !!}</td>
                                                ...
                                                <td class="kt-datatable__cell">
                                                 ...
                                                 <li class="kt-nav__item"><a href="{{url('gwc/survey/details/'.$surveyList->id)}}" class="kt-nav__link"><i class="kt-nav__link-icon flaticon-eye"></i><span class="kt-nav__link-text">{{__('adminMessage.view')}}</span></a></li>
                                                 @endif
                                                 
                                                
                                                 
                                                 @if(auth()->guard('admin')->user()->can('survey-delete'))
                                                 <li class="kt-nav__item"><a href="javascript:;" data-toggle="modal" data-target="#kt_modal_{{$surveyList->id}}" class="kt-nav__link"><i class="kt-nav__link-icon flaticon2-trash"></i><span class="kt-nav__link-text">{{__('adminMessage.delete')}}</span></a></li>
                                                 @endif
                                                 ...
                                                 <!--Delete modal -->
 <div class="modal fade" id="kt_modal_{{$surveyList->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
								<div class="modal-dialog" role="document">
									<div class="modal-content">
										...
										<div class="modal-footer">
											...
											<button type="button" class="btn btn-danger"  onClick="Javascript:window.location.href='{{url('gwc/survey/delete/'.$surveyList->id)}}'">{{__('adminMessage.yes')}}</button>
										</div>
									</div>
								</div>
							</div>
                                        ...
                                        @php $p++; @endphp
                                        @endforeach   
                                        <tr><td colspan="10" class="text-center">{{ $surveyLists->links() }}</td></tr> 
                                        @else
