...
<html lang="en">
	...
	<head>
		<base href="../../">
		<meta charset="utf-8" />
		<title>{{__('adminMessage.websiteName')}}|{{__('adminMessage.slider')}}</title>
						...
						<div class="kt-subheader   kt-grid__item" id="kt_subheader">
							<div class="kt-container  kt-container--fluid ">
								<div class="kt-subheader__main">
									<h3 class="kt-subheader__title">{{__('adminMessage.slider')}}</h3>
								...
								<div class="kt-subheader__toolbar">
									...
									<div class="btn-group">
										...
										<a href="{{url('gwc/slides/create')}}" class="btn btn-brand btn-bold"><i class="la la-plus"></i>&nbsp;{{__('adminMessage.createnew')}}</a>
							...
							<div class="kt-portlet kt-portlet--mobile">
                                ...
                                @if(auth()->guard('admin')->user()->can('services-list'))
									...
									<table class="table table-striped- table-bordered table-hover table-checkable " id="kt_table_1">
										...
										<tbody>
                                        @if(count($SlidesLists))
                                        @php $p=1; @endphp
                                        @foreach($SlidesLists as $slide)
											<tr class="search-body">
                                                ...
                                                <td class="kt-datatable__cell">
                                                 ...
                                                 <li class="kt-nav__item"><a href="{{url('gwc/slides/'.$slide->id.'/edit')}}" class="kt-nav__link"><i class="kt-nav__link-icon flaticon2-contract"></i><span class="kt-nav__link-text">{{__('adminMessage.edit')}}</span></a></li>
                                                 @endif
                                                 @if(auth()->guard('admin')->user()->can('services-delete'))
                                                 <li class="kt-nav__item"><a href="javascript:;" data-toggle="modal" data-target="#kt_modal_{{$slide->id}}" class="kt-nav__link"><i class="kt-nav__link-icon flaticon2-trash"></i><span class="kt-nav__link-text">{{__('adminMessage.delete')}}</span></a></li>
                                                 @endif
                                                 </ul>
                                                 </div>
                                                 </div>
                                                 ...
                                                 <!--Delete modal -->
 <div class="modal fade" id="kt_modal_{{$slide->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
								<div class="modal-dialog" role="document">
									<div class="modal-content">
										...
										<div class="modal-footer">
											...
											<button type="button" class="btn btn-danger"  onClick="Javascript:window.location.href='{{url('gwc/slides/delete/'.$slide->id)}}'">{{__('adminMessage.yes')}}</button>
										</div>
									</div>
								</div>
							</div>
                                        ...
                                        @php $p++; @endphp
                                        @endforeach   
                                        <tr><td colspan="8" class="text-center">{{ $SlidesLists->links() }}</td></tr>
                                        @else
