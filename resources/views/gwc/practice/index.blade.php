...
<html lang="en">
	...
	<head>
		<base href="../../">
		<meta charset="utf-8" />
		<title>{{__('adminMessage.websiteName')}}|{{__('adminMessage.practiceares')}}</title>
							...
							<div class="kt-container  kt-container--fluid ">
								<div class="kt-subheader__main">
									...
									<div class="kt-subheader__breadcrumbs">
										...
										<a href="javascript:;" class="kt-subheader__breadcrumbs-link">{{__('adminMessage.practiceareslisting')}}</a>
								...
								<div class="kt-subheader__toolbar">
									...
									<div class="btn-group">
                                        @if(auth()->guard('admin')->user()->can('practice-create'))
										<a href="{{url('gwc/practice/create')}}" class="btn btn-brand btn-bold"><i class="la la-plus"></i>&nbsp;{{__('adminMessage.createnew')}}</a>
                                        @endif
										...
										<div class="dropdown-menu dropdown-menu-right">
											<ul class="kt-nav">
												<li class="kt-nav__item">
													<a href="{{url('gwc/practice/pdf')}}" class="kt-nav__link">
							...
							<div class="kt-portlet kt-portlet--mobile">
								<div class="kt-portlet__head kt-portlet__head--lg">
									<div class="kt-portlet__head-label">
										...
										<h3 class="kt-portlet__head-title">
											{{__('adminMessage.practiceareslisting')}}
										</h3>
									</div>
								</div>
                                ...
                                @if(auth()->guard('admin')->user()->can('practice-list'))
									...
									<table class="table table-striped- table-bordered table-hover table-checkable" id="kt_table_1">
										...
										<tbody>
                                        @if(count($PracticeLists))
                                        @php $p=1; @endphp
                                        @foreach($PracticeLists as $PracticeList)
											<tr class="search-body">
												<td>{{$p}}</td>
												<td>{!! $PracticeList->title_en !!}</td>
												<td>{!! $PracticeList->title_en !!}</td>
												<td>
                                                @if($PracticeList->image)
                                                <img src="{!! url('uploads/practice/thumb/'.$PracticeList->image) !!}" width="40">
                                                @endif</td>
												<td>
                                                <span class="kt-switch"><label><input value="{{$PracticeList->id}}" {{!empty($PracticeList->is_active)?'checked':''}} type="checkbox"  id="practice" class="change_status"><span></span></label></span>
                                                </td>
												<td>{!! $PracticeList->created_at !!}</td>
												<td>{!! $PracticeList->updated_at !!}</td>
                                                <td class="kt-datatable__cell">
                                                 ...
                                                 <li class="kt-nav__item"><a href="{{url('gwc/practice/'.$PracticeList->id.'/edit')}}" class="kt-nav__link"><i class="kt-nav__link-icon flaticon2-contract"></i><span class="kt-nav__link-text">{{__('adminMessage.edit')}}</span></a></li>
                                                 @endif
                                                 @if(auth()->guard('admin')->user()->can('practice-delete'))
                                                 <li class="kt-nav__item"><a href="javascript:;" data-toggle="modal" data-target="#kt_modal_{{$PracticeList->id}}" class="kt-nav__link"><i class="kt-nav__link-icon flaticon2-trash"></i><span class="kt-nav__link-text">{{__('adminMessage.delete')}}</span></a></li>
                                                 @endif
                                                 </ul>
                                                 </div>
                                                 </div>
                                                 ...
                                                 <!--Delete modal -->
 <div class="modal fade" id="kt_modal_{{$PracticeList->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
								<div class="modal-dialog" role="document">
									<div class="modal-content">
										...
										<div class="modal-footer">
											...
											<button type="button" class="btn btn-danger"  onClick="Javascript:window.location.href='{{url('gwc/practice/delete/'.$PracticeList->id)}}'">{{__('adminMessage.yes')}}</button>
										</div>
									</div>
								</div>
							</div>
                                        ...
                                        @php $p++; @endphp
                                        @endforeach   
                                        <tr><td colspan="8" class="text-center">{{ $PracticeLists->links() }}</td></tr> 
                                        @else
